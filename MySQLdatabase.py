...
# Commands
settimezone = "select SET GLOBAL time_zone = 'Europe/Ljubljana';"
...
insertdata = """INSERT INTO %s (onewireID, source, timeframe, sensor, data, unit)
VALUES("%s", "%s", "%s", "%s", %.2f, "%s");"""

querydata = """SELECT * from %s where ttimestamp > current_timestamp() - interval %s and onewireID = "%s" and source = "%s" and timeframe = "%s" and sensor = "%s";"""
queryrecentdata = """SELECT max(ttimestamp), data from %s where ttimestamp > current_timestamp() - interval %s and onewireID = "%s" and source = "%s" and timeframe = "%s" and sensor = "%s";"""
...
# Sample commands
# SELECT * from sensordata where ttimestamp > current_timestamp() - interval 48 hour and onewireID = "1st Floor" and sensor = "temperature";
#
# INSERT INTO sensordata(onewireID, source, timeframe, sensor, data, unit)
...
# SELECT max(ttimestamp), data from sensordata where ttimestamp > current_timestamp() - interval 1 hour and onewireID = "1st Floor" and source = "Nest" and timeframe = "Current" and sensor = "Temperature";
...
def InsertData(db, table, onewireID, source, timeframe, sensor, data, unit):
	try:
		cmd = insertdata % (table, onewireID, source, timeframe, sensor, float(data), unit)
...
def QueryDataInterval(db, table, interval, onewireID, source, timeframe, sensor):
	try:
		query = querydata % (table, interval, onewireID, source, timeframe, sensor)
		cursor = db.cursor()
...
def QueryRecentData(db, table, interval, onewireID, source, timeframe, sensor):
	try:
		query = queryrecentdata % (table, interval, onewireID, source, timeframe, sensor)
		cursor = db.cursor()
		cursor.execute(query)
